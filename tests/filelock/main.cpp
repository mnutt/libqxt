...
class Job : public QObject
        ...
        public:
                ...
                void exec(QThread * onthread)
                        ...
                        emit(inwrap_s());
                        QVERIFY(QxtSignalWaiter::wait(this,SIGNAL(jobDone())));
                        }
        public slots:
                void inwrap_d()
                        {
...
class LockJob : public Job
        {
        public:
                LockJob(QxtFileLock*f,bool expectedresult){lock=f;expected=expectedresult;}
                QxtFileLock*lock;
                bool expected;
                virtual void run()
                        {
...
class QxtFileLockThreadTest : public QObject
        ...
        private slots:
                ...
                void ww_same()
                        {
                        QFile file1("lock.file");QVERIFY(file1.open(QIODevice::ReadWrite));
                        QFile file2("lock.file");QVERIFY(file2.open(QIODevice::ReadWrite));
